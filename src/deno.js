...
const handleUpdate = webhookCallback(bot, "std/http", {
  ...
  onTimeout: "throw",
  timeoutMilliseconds: config.webhookCallbackTimeoutMs,
});
await bot.api.setWebhook(config.webhookUrl, { secret_token: config.webhookSecret, allowed_updates: ["message"] });

/**
 * HTTP-обработчик Deno Deploy: маршрутизирует запросы health-check и webhook Telegram.
 *
 * @param {Request} request - Входящий HTTP-запрос.
 * @returns {Response|Promise<Response>} HTTP-ответ.
 */
